...
@app.route('/')
def hello_world():
    return render_template('home.html', template_folder='template')
...
def chat():
    try:
        ...
        if intent == "greet":
            response_text = random.choice(intent_response_dict[intent])
        if intent == "xtra":
            response_text = boss_info(entities)
        if intent == "query":
            response_text = boss_query(entities)
        if intent == "affirm":
            response_text = random.choice(intent_response_dict[intent])
        if intent == "goodbye":
            response_text = random.choice(intent_response_dict[intent])
